"""
Subsidized MDP solver (Algorithm 2).

Solves the agent's Markov decision process via vectorized backward induction over
the (N, X) state grid, tracking V^epsilon, V^0 (the value with no subsidy under the
same policy), and A (expected cost conditional on approval) exactly per the linear
decomposition of Proposition 7: V^epsilon = V^0 + epsilon * A.
"""
...
class SubsidizedMDPSolver:
    """Vectorized backward-induction solver for the subsidized sequential RCT MDP."""
    ...
    def __init__(self, rho_A, T, n_max, c0, c1, epsilon, kappa, theta_b,
        ...
        # Precomputed constants for the rejection condition (Eq. 30)
    ...
    def beta_binomial_pmf(self, x, n, alpha, beta):
        ...
        Inputs (`x` successes, `n` trials, `alpha`/`beta` belief parameters) may be
        any broadcastable PyTorch tensors.
        """
        log_pmf = (
    ...
    def check_rejection(self, N, X):
        """Rejection test via the linear reformulation of f(alpha, beta) >= 1/kappa (Eq. 30)."""
    ...
    def solve(self, save_dir='./mdp_results', chunk_size=512):
        ...
        Tracks V^epsilon, V^0, and A exactly per Proposition 7. Active (non-rejected)
        states are processed in chunks of ``chunk_size`` to bound peak GPU memory:
        the per-chunk PMF tensor has shape ``(k, n_max, n_max+1)``, so chunking
        controls the leading dimension at large ``n_max``.
        """
        os.makedirs(save_dir, exist_ok=True)

        # Absolute maximum trials N reachable across the horizon
        max_N_global = self.T * self.n_max

        results = {'V_eps': {}, 'V_0': {}, 'A': {}, 'P_approval': {}, 'Policy': {}}

        # Full-size "next-step" buffers so indexing into clamped states is safe
        ...
        costs_per_action = self.c0 + self.c1 * actions                                  # (n_max,)
        x_outcomes       = torch.arange(self.n_max + 1, dtype=torch.float32,
                                        device=self.device).view(1, 1, -1)              # (1, 1, n_max+1)

        for l in range(self.T, -1, -1):
            max_N_t = l * self.n_max

            # (N, X) grid for the current step l
            ...
            if active_mask.any():
                ...
                for cs in range(0, K, chunk_size):
                    ...
                    # State x action expansions: (k, n_max)
                    ...
                    # Belief parameters at (N, X): (k, n_max, 1)
                    alpha_ch = (self.alpha_0 + X_exp).unsqueeze(-1)
                    beta_ch  = (self.beta_0 + N_exp - X_exp).unsqueeze(-1)

                    A_ext        = A_exp.unsqueeze(-1).expand(-1, -1, self.n_max + 1)   # (k, n_max, n_max+1)
                    valid_x_mask = x_outcomes <= A_ext

                    pmf = torch.zeros(k, self.n_max, self.n_max + 1,
                    ...
                    # True next state (used for the rejection test)
                    ...
                    # Clamped indices for value-table lookup (safe because when
                    # next_rejected=False, next_N_true <= (l+1)*n_max <= max_N_global,
                    # so the clamp is a no-op; when next_rejected=True, the lookup
                    # value is immediately overwritten by the approval reward below).
                    next_N = next_N_true.clamp(0, max_N_global)
                    next_X = next_X_true.clamp(0, max_N_global)

                    # Accumulated cost C_l + cost(n) at the next step
                    ...
                    # Boundary conditions on approval
                    ...
                    # Expectation over outcomes x: (k, n_max)
                    ...
                    # Agent's Q-value for each action: -cost(n) + E[V_next]
                ...
                # Scatter chunked results back to the 2-D grids
            ...
            # Roll "current" values into the "next" buffers for step l-1
            V_eps_next.zero_()
            V_0_next.zero_()
            A_next.zero_()
            P_next.zero_()
        ...
        out_path = os.path.join(
            save_dir,
            f'mdp_results_eps_{self.epsilon:.3f}_T_{self.T}_nmax_{self.n_max}'
            f'_alpha_{self.alpha_0}_beta_{self.beta_0}_thetab_{self.theta_b}.pt'
        )
        torch.save(results, out_path)
        print(f"MDP solved. V^eps(0,0): {results['V_eps'][0][0,0]:.4f}, "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the subsidized MDP solver with a YAML config.")
    ...
    save_dir = config.get("save_dir") or "./mdp_output"
